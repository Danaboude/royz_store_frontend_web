...
export default function VendorDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    { name: t("vendor.dashboard"), href: "/vendor-dashboard", icon: Home },
    { name: t("sidebar.mySubscriptions"), href: "/vendor-dashboard/subscriptions", icon: CreditCard },
    { name: t("vendor.myProducts"), href: "/vendor-dashboard/products", icon: ShoppingCart },
    { name: t("vendor.myOrders"), href: "/vendor-dashboard/orders", icon: BarChart2 },
    { name: t("vendor.payments"), href: "/vendor-dashboard/payments", icon: CreditCard },
  ];
          ...
          <div className="relative flex flex-col flex-1 w-full max-w-xs bg-white">
            ...
            <div className="flex-1 h-0 pt-5 pb-4 overflow-y-auto">
              <div className="flex items-center flex-shrink-0 px-4 flex-col gap-2 py-4">
                <span className="text-xl font-bold text-gray-900">{t("header.vendorDashboard") || "Vendor Dashboard"}</span>
              ...
              <nav className="px-2 mt-5 space-y-1">
                {menuItems.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className={`flex items-center px-2 py-2 text-base font-medium rounded-md ${pathname === item.href ? "bg-gray-200 text-gray-900" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}
                  >
            ...
            <nav className="flex-1 px-2 space-y-1">
              {menuItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`flex items-center px-2 py-2 text-sm font-medium rounded-md ${pathname === item.href ? "bg-gray-200 text-gray-900" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}
                >
